...
**当前使用旧事件的系统**：
  - `WallManager`：发布墙壁碰撞事件（技能系统依赖撞墙触发）
  - `EffectManager`：订阅旧事件播放墙壁特效（已兼容新事件）
  - `DamageProcessor`：维护双接口（旧 `OnAttack` + 新 `ProcessDamage`）
  - `SkillManager`：订阅旧事件用于技能触发条件

**方案**：①保持现状（推荐）②完整重构（墙壁→CollisionEvent + 技能触发条件重构）  
**影响**：高影响，需重构技能系统撞墙触发逻辑。当前功能正常，架构上墙壁使用旧事件合理（环境交互 vs 战斗伤害）。  
**时机**：技能系统重构时一并处理，或在行为系统重构完毕后评估。**整体删除旧事件系统，不保留。**
