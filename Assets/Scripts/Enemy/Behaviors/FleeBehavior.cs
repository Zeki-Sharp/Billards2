...
public class FleeBehavior : BaseMovementBehavior
{
    ...
    public override Vector2 ExecuteMovement(Transform enemyTransform, Transform playerTransform, EnemyData enemyData)
    {
        ...
        // 检查是否需要接近玩家（如果离玩家太远）
        if (enemyData.fleeConfig.approachWhenFar && distanceToPlayer > enemyData.fleeConfig.approachDistance)
        {
            Debug.Log($"FleeBehavior: 玩家距离过远 ({distanceToPlayer} > {enemyData.fleeConfig.approachDistance})，向玩家移动");
            ...
            // 使用接近玩家的专用距离参数
            targetPosition = CalculateTargetPosition(enemyTransform.position, direction, enemyData.fleeConfig.approachMoveDistance);
            
            SetMoving(true);
            isApproaching = true; // 标记为接近模式
            Debug.Log($"FleeBehavior: 接近玩家，方向: {direction}, 目标位置: {targetPosition}, 移动距离: {enemyData.fleeConfig.approachMoveDistance}");
        ...
        {
            Debug.Log($"FleeBehavior: 玩家距离适中 ({distanceToPlayer} > {enemyData.fleeConfig.triggerDistance})，保持位置");
            SetMoving(false);
            isApproaching = false;
            return enemyTransform.position;
        }
        ...
        // 使用逃跑移动的专用距离参数
        targetPosition = CalculateTargetPosition(enemyTransform.position, direction, enemyData.fleeConfig.moveDistance);
    ...
    public override float GetCurrentMoveSpeed()
    {
        ...
        // 根据当前行为模式返回对应的速度
        return isApproaching ? cachedEnemyData.fleeConfig.approachSpeed : cachedEnemyData.fleeConfig.moveSpeed;
    }
}
